...
A little tip if, like me, you want to publish a **public scoped package on NPM**: instead of simply running `npm run publish`, run `npm run publish --access public`. This will avoid you having to go to the package settings after every release to set it back to public.
